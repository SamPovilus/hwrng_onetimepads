...
BASE64_RFC4648_WITH_SUPPLEMENTAL = BASE64_RFC4648 + "!-_=+|.?"
...
def create_pdf(output_path="output.pdf", text=""):
    ...
    SUPPORTED_CHARACTERS_LEN = len(SUPPORTED_CHARACTERS)
    documentation_lines = [f"encryption is (message+otp)%{SUPPORTED_CHARACTERS_LEN}, decrpytion is (message-otp)%{SUPPORTED_CHARACTERS_LEN}"]
    for documentation_line in documentation_lines:
        if y < MARGIN:
            break
        c.drawString(x + 4 * ((FONT_SIZE * 0.6)), y, documentation_line)  # Start key at the data column
    ...
    c.drawString(x + 4 * ((FONT_SIZE * 0.6) ), y, column_numbers)  # Shifted to align with data
    y -= (LINE_SPACING/LINE_SPACING)* FONT_SIZE

    # Print supported characters as a key starting at the same column as data
    key_lines = [SUPPORTED_CHARACTERS[i:i + MAX_CHARS_PER_LINE] for i in range(0, len(SUPPORTED_CHARACTERS), MAX_CHARS_PER_LINE)]
    for key_line in key_lines:
        if y < MARGIN:
            break
        c.drawString(x + 4 * (FONT_SIZE * 0.6), y, key_line)  # Start key at the data column
        y -= LINE_SPACING/2
...
if __name__ == "__main__":
    ...
    if args.charset == "base64+sup":
        SUPPORTED_CHARACTERS = BASE64_RFC4648_WITH_SUPPLEMENTAL
